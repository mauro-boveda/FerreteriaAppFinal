        ...
        <div class="card-body">
            <form action="/productos" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="rela_marcas" name="rela_marcas" required> {{-- Cambiado --}}
                        ...
                        @foreach ($marcas as $marca)
                            <option value="{{ $marca->id_marcas }}" {{ old('rela_marcas') == $marca->id_marcas ? 'selected' : '' }}> {{-- Cambiado --}}
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="rela_medidas" name="rela_medidas" required> {{-- Cambiado --}}
                        ...
                        @foreach ($medidas as $medida)
                            <option value="{{ $medida->id_medida }}" {{ old('rela_medidas') == $medida->id_medida ? 'selected' : '' }}> {{-- Cambiado --}}
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select" id="rela_rubro" name="rela_rubro" required>
                        ...
                        @foreach ($rubros as $rubro)
                            <option value="{{ $rubro->id_rubro }}" {{ old('rela_rubro') == $rubro->id_rubro ? 'selected' : '' }}>
                                {{ $rubro->descripcion }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="cantidad_actual" name="cantidad_actual" value="{{ old('cantidad_actual') }}" required min="0"> {{-- Cambiado --}}
                </div>
                <div class="mb-3">
                    <label for="precio_compra" class="form-label">Precio Compra</label> {{-- Añadido --}}
                    <input type="number" step="0.01" class="form-control" id="precio_compra" name="precio_compra" value="{{ old('precio_compra') }}" required min="0">
                </div>
                <div class="mb-3">
                    <label for="porcentaje_utilidad" class="form-label">Porcentaje Utilidad (%)</label> {{-- Añadido --}}
                    <input type="number" class="form-control" id="porcentaje_utilidad" name="porcentaje_utilidad" value="{{ old('porcentaje_utilidad') }}" required min="0" max="100">
                </div>
                <div class="mb-3">
                    <label for="precio_venta" class="form-label">Precio Venta</label> {{-- Cambiado --}}
                    <input type="number" step="0.01" class="form-control" id="precio_venta" name="precio_venta" value="{{ old('precio_venta') }}" required min="0"> {{-- Cambiado --}}
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control" id="cantidad_minima" name="cantidad_minima" value="{{ old('cantidad_minima') }}" required min="0">
                </div>
                <div class="mb-3">
